package com.codetaylor.mc.hookt.modules.hookt.rule.drop;

import com.codetaylor.mc.athenaeum.util.WeightedPicker;
import com.codetaylor.mc.hookt.modules.hookt.rule.data.*;
...
public class DropModifier {
  ...
  public List<ItemStack> modifyDrops(
    ...
    for (RuleDrop drop : rule.drops) {
      ...
      if (drop.selector.isValidCandidate(luckLevel, logFile, debug)) {
        int weight = drop.selector.weight.value + (luckLevel * drop.selector.weight.luckModifier);
    ...
    int dropCount = rule.dropCount.get(RANDOM, luckLevel);
    ...
    for (int i = 0; i < dropCount; i++) {
      ...
      int itemQuantity = ruleDrop.item.quantity.get(RANDOM, luckLevel);
      ...
      int xp = ruleDrop.xp.get(RANDOM, luckLevel);
